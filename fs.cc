 ...
 * @return vector<Path> a vector containing the new order of the circuits (maybe)
 */
vector<Path> Path::listdir() {
  return listdir(false);
}
/**
...
vector<Path> Path::listdir(bool recursive) {
  ...
  while((entry = readdir(dir)) != NULL) {
    ...
    if(recursive && filepath.is_dir()) {
      ...
      for(vector<Path>::iterator i=contents.begin(); i!=contents.end(); ++i)
...
Path Path::operator+(const string& add) {
  return (path.substr(path.size()-1) == "/") ? Path(path + add) : Path(path + "/" + add);
}
Path Path::operator+(const Path& obj) {
  return (path.substr(path.size()-1) == "/") ? Path(path + obj.path) : Path(path + "/" + obj.path);
}
