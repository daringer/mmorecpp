...
class ConfigDataKeeper {
 ...
 public:
  ...
  const T& get() const {
    if (!same_data_types<T>())
      throw IncompatibleDataTypes(
          "Data: " + tinfo + " Template(Keeper::get) was: " + typeid(T).name());
    return *((T*)storage);
  }
  ...
  void set(const T& value) {
    if (!same_data_types<T>())
      throw IncompatibleDataTypes(
          "Data: " + tinfo + " Template(Keeper::set) was: " + typeid(T).name());
  ...
  ConfigOption& set_default(const T& value) {
    if (!data.same_data_types<T>())
      throw IncompatibleDataTypes("Data has: " + data.verbose_type() +
                                  " Template(Option::set_default) was: " +
                                  typeid(T).name());
...
class ConfigGroup {
 public:
  ...
  template <class T>
  ConfigOption& new_option(const std::string& new_id, const std::string& desc,
    ...
    members[new_id] =
        new ConfigOption(new_id, desc, typeid(T).name(), short_cmd);
