...
void XNCBaseWindow::set_layout() {//const tCharList& cl) {
  wborder(_win, layout->at(0), layout->at(1), 
                layout->at(2), layout->at(3), 
                layout->at(4), layout->at(5), 
                layout->at(6), layout->at(7));
  update();
...
void XNCBaseWindow::change_size(const tCoords& new_pos, const tCoords& new_size) {
...
XNCStatusBar::XNCStatusBar(const uint& cols, const tCoords& pos, const tCoords& size) 
  : XNCBaseWindow(pos, size), content_cols(cols) { }
...
XNCScrolling::XNCScrolling(const uint& lines, 
    const tCoords& pos, const tCoords& size) 
