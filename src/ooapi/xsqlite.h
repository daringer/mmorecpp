...
namespace MM_NAMESPACE() {
  ...
  template <class T>
  struct bind_dispatch;

  template <>
  struct bind_dispatch<std::string> {
    static int apply(sqlite3_stmt* stmt, const std::string& val,
                     const int& idx) {
      return sqlite3_bind_text(stmt, idx, val.c_str(), val.length(),
                               SQLITE_TRANSIENT);
    }
  };
  template <>
  struct bind_dispatch<long> {
    static int apply(sqlite3_stmt* stmt, const long& val, const int& idx) {
      return sqlite3_bind_int64(stmt, idx, val);
    }
  };
  template <>
  struct bind_dispatch<int> {
    static int apply(sqlite3_stmt* stmt, const int& val, const int& idx) {
      return sqlite3_bind_int(stmt, idx, val);
    }
  };
  template <>
  struct bind_dispatch<double> {
  ...
  class XSQLite {
   public:
    ...
    bool init_update(const std::string& table, const tStringList& cols,
    ...
    template <class T>
    bool bind(const T& data, const int& idx = -1) {
   ...
   private:
    typedef enum { _ready, _inited, _finished } tSQLState;
    tSQLState state;
