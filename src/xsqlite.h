...
class XSQLite {
  public:
    ...
    // SELECT * FROM table LEFT JOIN t2 ON table.id = t2.id WHERE foo=blub GROUP_BY t2.bla ORDER_BY foo
    // SELECT {what} FROM {table} [{join}] [WHERE {where}] [GROUP_BY {group_by}] [ORDER_BY {order_by}]
    bool init_select(const std::string& table,
